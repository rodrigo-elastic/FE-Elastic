...
description: Idempotent sync of FE Copilot's MCP server, thirteen tools, and master agent into Elastic Agent Builder. Creates a .mcp connector pointing at the FE Copilot MCP endpoint, registers thirteen Agent Builder tools (nine specialist tools, the Sloane competitive comparison tool, the Auro orchestrator, the Carmen one-page proposal generator, and the Astrid deployment validator) referencing that connector, then creates the master agent that orchestrates them. Reads KIBANA_URL and KIBANA_API_KEY from settings; runs in dry-run mode (logs payloads only) when no key is configured. Override the public backend URL with BACKEND_BASE_URL (e.g., an ngrok forwarding URL) when Kibana is remote. Run with: PYTHONPATH=backend python -m scripts.sync_agent_builder.
date: 03-05-2026
"""
...
# Master agent instructions below now reference the Auro orchestrator, Sloane, Carmen, Astrid, and the thirteen-tool catalogue.
...
You have thirteen specialized tools, each backed by a dedicated expert persona or pure-compute helper:
...
def build_agent_payload() -> Dict[str, Any]:
    return {
        ...
        "description": "Elastic Field Engineering Assistant. Wraps the thirteen FE Copilot tools (POC plan, SPL to ES|QL, compliance mapping, stack extract, code sample, cost calc, capacity planner, docs knowledge search, troubleshooter, Sloane competitive comparison, Auro orchestrator, Carmen one-page proposal generator, Astrid deployment validator).",
        "configuration": {
