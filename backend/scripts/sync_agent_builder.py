...
description: Idempotent sync of FE Copilot's MCP server, fourteen tools, and master agent into Elastic Agent Builder. Creates a .mcp connector pointing at the FE Copilot MCP endpoint, registers fourteen Agent Builder tools (nine specialist tools, the Sloane competitive comparison tool, the Auro orchestrator, the Carmen one-page proposal generator, the Astrid deployment validator, and the Lina POV health monitor) referencing that connector, then creates the master agent that orchestrates them. Reads KIBANA_URL and KIBANA_API_KEY from settings; runs in dry-run mode (logs payloads only) when no key is configured. Override the public backend URL with BACKEND_BASE_URL (e.g., an ngrok forwarding URL) when Kibana is remote. Run with: PYTHONPATH=backend python -m scripts.sync_agent_builder.
date: 03-05-2026
"""
...
MCP_TOOLS: List[Dict[str, str]] = [
    ...
    {
        "id": "fec_deploy_validator",
        "name": "FE Copilot - Deployment Validator (Astrid)",
    ...
    {
        "id": "fec_pov_health",
        "name": "FE Copilot - POV Health Monitor (Lina)",
        "description": "Take a pasted POV/trial summary and emit stage_assessment (on_track/at_risk/stalled), confidence_score, top 3 strengths, top 3 risks, and 4 owner-assignable next-best-actions. Tied directly to FE quota outcomes (POV conversion = revenue). Persona: Lina, Senior POV Operations Lead at Elastic (9 years running technical POVs, wrote the internal POV Health checklist).",
    },
]
# Master agent instructions below now reference the Auro orchestrator, Sloane, Carmen, Astrid, Lina, and the fourteen-tool catalogue.
...
You have fourteen specialized tools, each backed by a dedicated expert persona or pure-compute helper:
...
- fec_deploy_validator: Astrid (Senior Platform Architect, 12y on production Elasticsearch) audits a pasted cluster summary and emits a prioritized antipattern report with concrete remediation steps and a 0 to 100 cluster health score. Use fec_deploy_validator when the user pastes a deployment summary or asks to validate a cluster's configuration, sizing, ILM, security posture, ingest pipelines, snapshots, JVM heap, or shard counts.
- fec_pov_health: Lina (Senior POV Operations Lead, 9y running technical POVs) audits a pasted POV/trial cluster summary and emits a stage_assessment (on_track/at_risk/stalled), confidence_score, top 3 strengths, top 3 risks, and 4 owner-assignable next-best-actions. Use fec_pov_health when the user pastes a trial or POV cluster summary, asks 'how is this trial doing', wants to flag a trial at risk, or asks for a weekly POV health check.

Pick the right tool for each request. Use fec_compare when the user asks about Elastic vs a specific competitor for either technical or pricing comparison. When the user asks for a 'proposal', 'one-page proposal', '1-page proposal', 'pursuit document', '1-page summary for the customer', 'customer proposal', 'one-pager', or wants to attach a deliverable to a Salesforce opportunity, you MUST call fec_proposal. If the user names a customer (e.g., 'Banco Atlántico', 'Northwind Pay', 'Mercado Atlas') but does not provide an explicit meeting_id, pass the customer slug + '-mtg-prev-001' as the meeting_id (atlantico-mtg-prev-001, northwind-mtg-prev-001, mercadoatlas-mtg-prev-001) so Carmen can ground the proposal in the most recent meeting on disk. If the user does not name a customer at all, you MUST still call fec_proposal immediately with meeting_id='atlantico-mtg-prev-001' (the canonical demo meeting). Never ask a clarifying question for a proposal request; always call fec_proposal first with the best inferred meeting_id, the FE will override after seeing the output. Do not ask the user to supply a meeting_id; infer it. Use fec_orchestrator when the user asks something that requires 2-3 tools chained, OR when you would otherwise call more than 2 tools yourself; let Auro plan it. Combine tools yourself only for simple two-tool combinations (e.g., compliance + cost calc for a security POV; knowledge search to ground a POC plan in current docs; troubleshoot then knowledge search to confirm a remediation). Use fec_knowledge_search whenever the user asks a product-specific question that the public Elastic docs would answer (sizing, ILM, ES|QL syntax, semantic_text setup, detection rules). Use fec_troubleshoot when the user pastes an error message, log snippet, or describes a stack issue that needs diagnosis. Use fec_deploy_validator when the user pastes a deployment summary or asks to validate a cluster's configuration, sizing, ILM, security posture, ingest pipelines, snapshots, JVM heap, or shard counts. Use fec_pov_health when the user pastes a trial or POV cluster summary, asks 'how is this trial doing', or wants to flag a trial at risk. Always be honest about gaps, never invent customer-specific details. Never use the em dash character."""
...
def build_agent_payload() -> Dict[str, Any]:
    return {
        ...
        "description": "Elastic Field Engineering Assistant. Wraps the fourteen FE Copilot tools (POC plan, SPL to ES|QL, compliance mapping, stack extract, code sample, cost calc, capacity planner, docs knowledge search, troubleshooter, Sloane competitive comparison, Auro orchestrator, Carmen one-page proposal generator, Astrid deployment validator, Lina POV health monitor).",
        "configuration": {
