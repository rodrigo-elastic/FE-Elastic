...
**Complementary, not duplicate.** The 31 battlecards in `data/seed/battlecards.json`, the 20 industry profiles in `data/seed/industries.json`, and the synthetic renewal-risk signals are demo scaffolding for the video. Production path: read from the source-of-truth systems Elastic FEs already trust, rather than re-author that research here. The MCP surface (`fec_compare`, `fec_proposal`, `fec_orchestrator`) is identical in both modes; only the data source changes. See `docs/architecture.md` section "Complementary integration with existing FE tooling" for the planned `seismic_battlecard_lookup`, `highspot_doc_search`, `sfdc_account_snapshot`, `gainsight_health`, and `slack_search` tools, plus the swap matrix per in-repo seed. A submission that overrides Seismic's competitive content would be rejected by every FE who trusts it; FE Copilot's role is to be the synthesizer that fans out to those systems, layered with Elastic-specific math and meeting context, with audit-grade provenance back to each upstream source.
